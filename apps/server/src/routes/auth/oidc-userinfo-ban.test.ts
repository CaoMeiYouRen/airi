...
describe('auth UI routes', () => {
  it('redirects sign-in provider shortcut to the standalone auth UI', async () => {
    ...
    expect(location).toBe('https://auth.airi.build/auth/sign-in?provider=github&client_id=stage-web&prompt=login&redirect_uri=http%3A%2F%2Flocalhost%3A5173%2Fauth%2Fcallback')
  })
  ...
  it('redirects Electron OIDC callback queries to the standalone auth UI relay', async () => {
    ...
    expect(res.status).toBe(302)
    expect(res.headers.get('location')).toBe('https://auth.airi.build/auth/api/auth/oidc/electron-callback?code=sample-code&state=43123%3Aopaque-state')
  })
})
